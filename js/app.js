...
/* ---------- Plan de révision hebdomadaire en 5 colonnes ---------- */
function rendrePlanSemaine() {
  const pp = chargerPlanning();
  const taches = pp.taches || {};
  const total = Object.values(taches).flat().length;
  if (total === 0) {
    return `<div class="edt-vide">
      <p>Aucune séance planifiée pour l'instant. Remplis ton plan semaine par semaine !</p>
      <a class="btn btn-primaire btn-petit" href="#/planning">➕ Remplir mon plan de révision</a>
    </div>`;
  }
  const faites = Object.values(taches).flat().filter(t => t.fait).length;
  const pct = Math.round(100 * faites / total);
  const colonnes = JOURS_EDT.slice();
  if ((taches["Samedi"] || []).length + (taches["Dimanche"] || []).length > 0) colonnes.push("Week-end");
  return `
    <div class="planning-recap"><div class="planning-barre"><div class="rempli" style="width:${pct}%"></div></div><span>${faites}/${total} séance${total > 1 ? "s" : ""} faite${faites > 1 ? "s" : ""}</span></div>
    <div class="planning-grille">
      ${colonnes.map(jour => {
        let liste;
        if (jour === "Week-end") {
          liste = [...(taches["Samedi"] || []).map(t => ({ ...t, jour: "Samedi" })), ...(taches["Dimanche"] || []).map(t => ({ ...t, jour: "Dimanche" }))];
        } else {
          liste = (taches[jour] || []).map(t => ({ ...t, jour }));
        }
        const nFaites = liste.filter(t => t.fait).length;
        return `
        <div class="jour-carte" data-plan-jour="${jour}">
          <div class="jour-tete"><b>${jour}</b><span class="jour-compte">${nFaites}/${liste.length}</span></div>
          ${liste.length === 0 ? `<div class="jour-vide">—</div>` : `
          <ul class="jour-taches">
            ${liste.map((t, i) => {
              ...
              return `<li class="tache ${t.fait ? "faite" : ""}">
                <button class="tache-coche" data-plan-toggle="${t.jour}|${t.id}" title="Marquer faite">${t.fait ? "✓" : ""}</button>
    ...
    <div style="text-align:center;margin-top:12px"><a class="btn btn-secondaire btn-petit" href="#/planning">➕ Modifier mon plan de révision</a></div>`;
}

function brancherPlanSemaine() {
  document.querySelectorAll("[data-plan-toggle]").forEach(b => b.addEventListener("click", () => {
    const [jour, id] = b.dataset.planToggle.split("|");
    const pp = chargerPlanning();
    pp.taches = pp.taches || {};
    const t = (pp.taches[jour] || []).find(x => x.id === id);
    if (!t) return;
    t.fait = !t.fait;
...
/* ---------- Accueil : ambiance ---------- */
const PHRASES_JOUR = [
  "Un petit pas chaque jour construit un grand chemin.",
  "Bien réviser, c'est comprendre une fois et revoir au bon moment.",
  "Organise ton travail : ton travail s'occupera de toi.",
  "Cinq minutes ce soir valent une heure la veille de l'examen.",
  "Comprendre est la moitié du chemin — réviser est l'autre moitié.",
  "Ce que tu écris ta main le retient ; ce que tu expliques ton cœur le garde.",
  "Une leçon relue le soir est une leçon à moitié apprise pour toujours.",
  "La régularité gagne toujours contre le hasard.",
  "Chaque case cochée est une promesse tenue à toi-même.",
  "Apprendre aujourd'hui, c'est se simplifier demain."
];
function phraseDuJour() {
  const d = new Date();
  const jour = Math.floor((d - new Date(d.getFullYear(), 0, 0)) / 86400000);
  return PHRASES_JOUR[jour % PHRASES_JOUR.length];
}
function salutation() {
...
function pageAccueil() {
  ...
  const dateLongue = aujourdhui.toLocaleDateString("fr-FR", { weekday: "long", day: "numeric", month: "long" });

  APP.innerHTML = `
    <div class="accueil-bonjour">
      <h1>${salutation()} Hidaya <span class="petit-coeur">💗</span></h1>
      <p class="bonjour-date">${dateLongue}</p>
      <p class="phrase-jour">« ${phraseDuJour()} »</p>
    </div>
    ...
    <h2 class="titre-section" style="margin-top:26px">📋 Mon plan de révision de la semaine</h2>
    ${rendrePlanSemaine()}`;
      })()}
    </section>
    ...
    <h2 class="titre-section">Toutes mes matières</h2>
...
/* ---------- Sommaire dépliable du programme ----------
   Chaque grande unité a sa couleur ; un clic déplie la liste des leçons,
   chaque leçon étant cliquable et marquée de son statut. */
const PALETTE_UNITES = ["#2563eb", "#dc2626", "#059669", "#ea580c", "#7c3aed", "#db2777", "#0d9488", "#ca8a04", "#65a30d", "#475569", "#0369a1", "#9333ea"];

function sommaireMatiere(mid) {
  const lecons = domainesDe(mid).flatMap(d => leconsDuDomaine(d));
  const themes = [];
  lecons.forEach(l => {
  ...
  return `<div class="sommaire">
    ${themes.map((t, i) => {
      const c = PALETTE_UNITES[i % PALETTE_UNITES.length];
      const faites = t.lecons.filter(l => statutLecon(l) === "terminee").length;
      return `
      <div class="sm-item" style="--uc:${c}">
        <button class="sm-tete ${i === 0 ? "ouvert" : ""}" data-sm="${i}">
          <span class="sm-num">${i + 1}</span>
          <span class="sm-nom" dir="auto">${esc(t.nom)}</span>
          <span class="sm-meta">${faites}/${t.lecons.length}</span>
          <span class="sm-chev"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="m9 18 6-6-6-6"/></svg></span>
        </button>
        <div class="sm-lecons" ${i === 0 ? "" : "hidden"}>
          ${t.lecons.map(l => {
            const st = statutLecon(l);
            return `<a class="sm-lecon st-${st}" dir="auto" href="#/lecon/${l.id}">
              <b>${l.tag ? l.tag : ""}</b><span class="sm-lecon-titre">${esc(l.titre)}</span>
              <span class="sm-st">${STATUTS[st].icone}</span>
            </a>`;
          }).join("")}
        </div>
      </div>`;
...
function brancherSommaire(cont) {
  cont.querySelectorAll(".sm-tete").forEach(b => {
    b.addEventListener("click", () => {
      const lecons = b.nextElementSibling;
      if (!lecons) return;
      const ouvert = !lecons.hidden;
      lecons.hidden = ouvert;
...
function pageMatiere(idMatiere) {
  ...
  APP.innerHTML = `
    ...
    <h2 class="titre-section">📑 Le sommaire du programme</h2>
    <p class="sous-titre" style="margin-top:-8px">Clique sur une unité pour déplier ses leçons — <span class="sm-legende st-terminee">✓ terminée</span> <span class="sm-legende st-encours">◐ en cours</span> <span class="sm-legende st-aucune">à faire</span></p>
    <div id="sommaire-matiere">${sommaireMatiere(idMatiere)}</div>
  ...
  `;
  brancherSommaire(document.getElementById("sommaire-matiere"));
}
